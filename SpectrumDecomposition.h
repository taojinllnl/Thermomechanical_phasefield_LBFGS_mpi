...
namespace usr_spectrum_decomposition
{
  ...
  template <int dim>
  double p_trace_strain_positive_p_temperature(SymmetricTensor<2, dim> const & strain_e,
		   			       const double alpha)
  {
  ...
  template <int dim>
  double p_trace_strain_negative_p_temperature(SymmetricTensor<2, dim> const & strain_e,
		   			       const double alpha)
  {
    double I_1 = trace(strain_e);
    if (I_1 < 0)
  ...
  template <int dim>
  SymmetricTensor<2, dim> p_strain_positive_p_temperature(Vector<double> const & eigenvalues,
					                  std::vector<Tensor<1, dim>> const & eigenvectors,
							  const double alpha)
  {
    ...
    for (int i = 0; i < dim; i++)
      {
	if (eigenvalues[i] > 0)
	  strain_positive_temperature -= alpha
	                               * symmetrize(outer_product(eigenvectors[i],
	                                                          eigenvectors[i]));
      }
  ...
  template <int dim>
  SymmetricTensor<2, dim> p_strain_negative_p_temperature(Vector<double> const & eigenvalues,
					                  std::vector<Tensor<1, dim>> const & eigenvectors,
							  const double alpha)
  {
    ...
    for (int i = 0; i < dim; i++)
      {
	if (eigenvalues[i] < 0)
	  strain_negative_temperature -= alpha
	                               * symmetrize(outer_product(eigenvectors[i],
	                                                          eigenvectors[i]));
      }
